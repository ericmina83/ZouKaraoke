...
class Song:
    ...
    def output_csv(self):
        return (
            ...
            + self.name
            + ","
            + self.get_singers_name()
            + ","
            + singerTypes[self.singerType]
            + ","
            + self.path
        )
...
def check_filename(basename: str, extension: str) -> SongVersion | None:
    ...
    for singerName in singerNames:
        ...
        singer = next(
            (
                tmpSinger
                for tmpSinger in singers
                if singerName.upper() in tmpSinger.name.upper()
                or tmpSinger.name.upper() in singerName.upper()
            ),
            None,
        )
    ...
    if id_song == None:
        ...
        song = next(
            (
                tmpSong
                for tmpSong in songs
                if songName.upper() in tmpSong.name.upper()
                or singer.name.upper() in songName.upper()
            ),
            None,
        )
        ...
        if song == None:
            ...
            song = Song(
                sn,
                targetSingers_maxSongId[targetSingersStr],
                targetSingers,
                songName,
                langType,
                singerType,
        ...
        if song == None:
            song = Song(sn, songId, targetSingers, songName, langType, singerType)
        songAlreadyExists = True
